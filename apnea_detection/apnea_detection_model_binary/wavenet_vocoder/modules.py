...
class Conv1d1x1_v2(nn.Module):
    """1-by-1 convolution implemented using Linear Layer
    """
    def __init__(self, in_channels, out_channels, bias=True):
        super(Conv1d1x1_v2, self).__init__()
        self.linear = nn.Linear(in_channels, out_channels, bias=bias)

    def forward(self, x):
        N = x.shape[0]
        # x.shape = (minibatch_size, #inch, T)
        x = x.permute(0,2,1)
        # x.shape = (minibatch_size, T, #inch)
        x = x.reshape(-1, x.shape[-1])
        # x.shape = (minibatch_sizexT, #inch)
        x = self.linear(x)
        # x.shape = (minibatch_sizexT, #outputch)
        x = x.reshape(N, -1, x.shape[-1])
        # x.shape = (minibatch_size, T, #outputch)
        x = x.permute(0,2,1)
        # x.shape = (minibatch_size, #outputch, T)
        return x
        ...
        if padding is None:
            ...
            if causal:
                ...
                if dilation % 2 == 1:
                    ...
                    padding_left = int(np.ceil(padding_total/2.))
...
if __name__=='__main__':
    conv1 =  Conv1d1x1(1, 32, bias=False)
    conv2 = Conv1d1x1_v2(1, 32, bias=False)

    conv2.linear.weight.data = conv1.weight.data[...,0]
    
    conv1 = conv1.cuda()
    conv2 = conv2.cuda()

    x = torch.Tensor(np.random.randn(64,1,4100).astype(float)).cuda()
