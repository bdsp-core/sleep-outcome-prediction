...
from modules import Conv1d1x1, Conv1d1x1_v2, ResidualConv1dGLU, ConvTranspose2d
...
class WaveNet(nn.Module):
    ...
    def __init__(self, out_channels=5, layers=12, stacks=1,
        ...
        if scalar_input:
            self.first_conv = Conv1d1x1_v2(1, residual_channels)
        else:
            self.first_conv = Conv1d1x1_v2(out_channels, residual_channels)
        ...
        self.last_conv_layers = nn.ModuleList([
            ...
            Conv1d1x1_v2(skip_out_channels, skip_out_channels),
            nn.ReLU(inplace=True),
            Conv1d1x1_v2(skip_out_channels, out_channels),
        ])
    ...
    def incremental_forward(self, initial_input=None, c=None, g=None,
                            ...
                            tqdm=lambda x: x, softmax=False, quantize=True,
                            log_scale_min=-50.0):
...
if __name__=='__main__':
    ...
    # do some experiments to figure out
    # the misalignment between receptive field and the input segment
    # test if causal/non-causal work. error is induced to observe change from middle of
    # receptive field. WORKS.
    ...
    x = np.random.randn(1,1,4100).astype('float32')*10

    x2 = x.copy()
    x2[0,0,-1] = - 100000  # change input sample 
    ...
    model = WaveNet(out_channels=5, layers=12, stacks=1,
                 residual_channels=32,
                 gate_channels=32,
                 skip_out_channels=32,
                 kernel_size=2, dropout=0.2,
    ...
    # print(y[0,0,4040:4060])
    # print(y[0, 0, -10:])
    y = y.cpu().detach().numpy().astype(float)
    y2 = y2.cpu().detach().numpy().astype(float)

    # print(y[0,0,:]==y2[0,0,:])
    print(np.where(np.logical_not(y[0,0,:]==y2[0,0,:]))[0][0]) # find location of first changed output sample
    print(y[0, 0, 2040:2060])
    print(y2[0, 0, 2040:2060])
