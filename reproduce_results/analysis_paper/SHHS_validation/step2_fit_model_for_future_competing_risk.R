...
outcome <- 'Myocardial_Infarction'#args[1]
sleep_stage_type <- 'NREM'#args[2] #'AHI', '5stage', 'NREM'
#if (length(args)>=3) {
...
if (sensitivity_analysis=='none') {
  result_path <- file.path(base_folder, sprintf('SHHS_validation/survival_results_%s_MGH_central', sleep_stage_type))
} else {
  result_path <- file.path(base_folder, sprintf('SHHS_validation/survival_results_%s_%s_MGH_central', sleep_stage_type, sensitivity_analysis))
}
...
dfX <- read.csv(file.path(base_folder, 'SHHS_features_NREM.csv'))
...
save_path <- file.path(result_path, sprintf('AJ_output_%s_%s_overall.mat', outcome, sleep_stage_type))
...
# restrict to central for SHHS
xnames_mask <- sapply(xnames, function(x) grepl('(_C$|_C_)',x))
xnames <- xnames[xnames_mask]
  ...
  tryCatch(  # some bootstrap will throw error, if so, ignore and continue
    expr = {
      ...
      for (cvi in 1:Ncv) { # outer loop
        ...
        if (bti==1) {
          ...
          if (sleep_stage_type!='AHI') {
            ...
            for (alpha_ in alpha_list) {
              print(alpha_)
              coxph_cv_fit <- fit_cv_cox_model(Xtr, ytr, list(xnames_c), foldid, alpha_, n.core=Ncv)
            ...
            coef_ <- get_cv_cox_coef(best_coxph_cv_fit, list(xnames_mask_c))
          } else {
        ...
        if (sleep_stage_type!='AHI') {
          coxph_fit <- fit_model(dftr, list(xnames2_c))
        ...
        #if ((sensitivity_analysis=='male')|(sensitivity_analysis=='female')) {
        #  rep_val <- c(age_mean, bmi_mean, benzo, antidep, sedative, antieplipetic, stimulant)
        #} else {
        rep_val <- c(age_mean, sex_mean, bmi_mean, benzo, antidep, sedative, antieplipetic, stimulant)
        #}
        actual.sex.tr <- dftr$Sex
        actual.sex.te <- dfte$Sex
        dftr[,covariate_names] <- t(replicate(nrow(dftr), (rep_val-Xmean_covariate_names)/Xstd_covariate_names))
        dfte[,covariate_names] <- t(replicate(nrow(dfte), (rep_val-Xmean_covariate_names)/Xstd_covariate_names))
        ...
        z_boundaries <- quantile(zptr, probs=c(0.25,0.75))  # use boundaries based on training set
...
if (sleep_stage_type!='AHI') {
  ...
  coxph_cv_fit <- fit_cv_cox_model(X, y_, list(xnames_c), foldid, best_alpha, n.core=Ncv)
  coef_ <- get_cv_cox_coef(coxph_cv_fit, list(xnames_mask_c))
  xnames2 <- c(xnames[abs(coef_)>0], covariate_names)
} else {
  xnames2 <- covariate_names
}
...
# save coefficients
if (sleep_stage_type!='AHI') {
  coxph_fit  <- fit_model(df, list(xnames2_c))
  coxph_coef <- get_coef(coxph_fit, xnames2)
} else {
...
writeMat(file.path(result_path, sprintf('results_%s_%s.mat', outcome, model_type)),
         ...
         zptr=zp_all,
         cindex_te=cindex_tes_bt, auc_te=auc_tes_bt,
