...
base_folder <- '/data/cdac Dropbox/a_People_BIDMC/Haoqi/SleepBasedOutcomePrediction/code-haoqi'
Ncv <- 5
Nbt <- 1000
random_seed <- 2021
...
outcome <- args[1]
exposure_type <- args[2] #'AHI', 'HB', 'RED', 'BAI', 'SE', 'REMPerc', 'WASO', 'NREM'
if (length(args)>=3) {
...
dfX <- read.csv(file.path(base_folder, 'MGH_features_NREM.csv'))
...
if (exposure_type=='AHI') {
  xnames <- c('AHI')
} else if (exposure_type=='HB') {
  xnames <- c('hypoxia_burden')
} else if (exposure_type=='RED') {
  xnames <- c('RED')
} else if (exposure_type=='BAI') {
  xnames <- c('BAI')
} else if (exposure_type=='SE') {
  xnames <- c('SleepEfficiency')
} else if (exposure_type=='REMPerc') {
  xnames <- c('REMPercTST')
} else if (exposure_type=='WASO') {
  xnames <- c('WASO')
} else {
  remove_names <- c(ynames, 'MRN', 'PatientID', 'CVFold', 'id', covariate_names, 'AHI', 'HB', 'RED', 'BAI', 'SE', 'REMPerc', 'WASO')
...
if (exposure_type=='AHI') {
  ...
  df <- df[!is.na(df$hypoxia_burden),]
} else if (exposure_type=='RED') {
  df <- df[!is.na(df$RED),]
} else if (exposure_type=='BAI') {
  df <- df[!is.na(df$BAI),]
} else if (exposure_type=='SE') {
  df <- df[!is.na(df$SleepEfficiency),]
} else if (exposure_type=='REMPerc') {
  df <- df[!is.na(df$REMPercTST),]
} else if (exposure_type=='WASO') {
  df <- df[!is.na(df$WASO),]
}
    ...
    expr = {
      ...
      for (cvi in 1:Ncv) { # outer loop
        ...
        if (bti==1) {
          ...
          if (exposure_type %in% c('AHI', 'HB', 'RED', 'BAI', 'SE', 'REMPerc', 'WASO')) {
        ...
        cindex_te <- get.cindex(model_type, dfte, xnames2, coxph_coef)
        ...
        if (exposure_type=='AHI') {
          ...
          zptr2[dftr$AHI<(boundaries[1]-Xmean[['AHI']])/Xstd[['AHI']]] <- 1
          zptr2[dftr$AHI>(boundaries[2]-Xmean[['AHI']])/Xstd[['AHI']]] <- 3
          zpte2 <- rep(2, nrow(dfte))
          zpte2[dfte$AHI<(boundaries[1]-Xmean[['AHI']])/Xstd[['AHI']]] <- 1
          zpte2[dfte$AHI>(boundaries[2]-Xmean[['AHI']])/Xstd[['AHI']]] <- 3
        } else if (exposure_type=='HB') {
          levels <- c(1,2,3)
          boundaries <- c(1,15)  # Q1, Q3
          zptr2 <- rep(2, nrow(dftr))
          zptr2[dftr$hypoxia_burden<(boundaries[1]-Xmean[['hypoxia_burden']])/Xstd[['hypoxia_burden']]] <- 1
          zptr2[dftr$hypoxia_burden>(boundaries[2]-Xmean[['hypoxia_burden']])/Xstd[['hypoxia_burden']]] <- 3
          zpte2 <- rep(2, nrow(dfte))
          zpte2[dfte$hypoxia_burden<(boundaries[1]-Xmean[['hypoxia_burden']])/Xstd[['hypoxia_burden']]] <- 1
          zpte2[dfte$hypoxia_burden>(boundaries[2]-Xmean[['hypoxia_burden']])/Xstd[['hypoxia_burden']]] <- 3
        } else if (exposure_type=='RED') {
          levels <- c(1,2,3)
          boundaries <- c(15,25)
          zptr2 <- rep(2, nrow(dftr))
          zptr2[dftr$RED<(boundaries[1]-Xmean[['RED']])/Xstd[['RED']]] <- 1
          zptr2[dftr$RED>(boundaries[2]-Xmean[['RED']])/Xstd[['RED']]] <- 3
          zpte2 <- rep(2, nrow(dfte))
          zpte2[dfte$RED<(boundaries[1]-Xmean[['RED']])/Xstd[['RED']]] <- 1
          zpte2[dfte$RED>(boundaries[2]-Xmean[['RED']])/Xstd[['RED']]] <- 3
        } else if (exposure_type=='BAI') {
          levels <- c(1,2,3)
          boundaries <- c(-1, 5.7)
          zptr2 <- rep(2, nrow(dftr))
          zptr2[dftr$BAI<(boundaries[1]-Xmean[['BAI']])/Xstd[['BAI']]] <- 1
          zptr2[dftr$BAI>(boundaries[2]-Xmean[['BAI']])/Xstd[['BAI']]] <- 3
          zpte2 <- rep(2, nrow(dfte))
          zpte2[dfte$BAI<(boundaries[1]-Xmean[['BAI']])/Xstd[['BAI']]] <- 1
          zpte2[dfte$BAI>(boundaries[2]-Xmean[['BAI']])/Xstd[['BAI']]] <- 3
        } else if (exposure_type=='SE') {
          levels <- c(1,2,3)
          boundaries <- c(70,90)
          zptr2 <- rep(2, nrow(dftr))
          zptr2[dftr$SleepEfficiency<(boundaries[1]-Xmean[['SleepEfficiency']])/Xstd[['SleepEfficiency']]] <- 1
          zptr2[dftr$SleepEfficiency>(boundaries[2]-Xmean[['SleepEfficiency']])/Xstd[['SleepEfficiency']]] <- 3
          zpte2 <- rep(2, nrow(dfte))
          zpte2[dfte$SleepEfficiency<(boundaries[1]-Xmean[['SleepEfficiency']])/Xstd[['SleepEfficiency']]] <- 1
          zpte2[dfte$SleepEfficiency>(boundaries[2]-Xmean[['SleepEfficiency']])/Xstd[['SleepEfficiency']]] <- 3
        } else if (exposure_type=='REMPerc') {
          levels <- c(1,2,3)
          boundaries <- c(10,20)
          zptr2 <- rep(2, nrow(dftr))
          zptr2[dftr$REMPercTST<(boundaries[1]-Xmean[['REMPercTST']])/Xstd[['REMPercTST']]] <- 1
          zptr2[dftr$REMPercTST>(boundaries[2]-Xmean[['REMPercTST']])/Xstd[['REMPercTST']]] <- 3
          zpte2 <- rep(2, nrow(dfte))
          zpte2[dfte$REMPercTST<(boundaries[1]-Xmean[['REMPercTST']])/Xstd[['REMPercTST']]] <- 1
          zpte2[dfte$REMPercTST>(boundaries[2]-Xmean[['REMPercTST']])/Xstd[['REMPercTST']]] <- 3
        } else if (exposure_type=='WASO') {
          levels <- c(1,2,3)
          boundaries <- c(15,70)
          zptr2 <- rep(2, nrow(dftr))
          zptr2[dftr$WASO<(boundaries[1]-Xmean[['WASO']])/Xstd[['WASO']]] <- 1
          zptr2[dftr$WASO>(boundaries[2]-Xmean[['WASO']])/Xstd[['WASO']]] <- 3
          zpte2 <- rep(2, nrow(dfte))
          zpte2[dfte$WASO<(boundaries[1]-Xmean[['WASO']])/Xstd[['WASO']]] <- 1
          zpte2[dfte$WASO>(boundaries[2]-Xmean[['WASO']])/Xstd[['WASO']]] <- 3
        } else {
          levels <- c(1,2,3)
...
write.csv(df, file.path(result_path, sprintf('df_%s_%s.csv', outcome, model_type)), row.names=F)
